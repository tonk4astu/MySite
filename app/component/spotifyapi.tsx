...
async function GetSpotifyMyData() {
  ...
  const token: acessToken = await fetch(
    `https://accounts.spotify.com/api/token`,
    {
      method: 'POST',
      headers: {
        Authorization: `Basic ${Buffer.from(clientId + ':' + secretId).toString(
          'base64',
        )}`,
...
async function FavoriteMusic(data: SpotifyData) {
  if (data.items !== undefined) {
    return data.items.map((Data, index) => {
      const likesinger: string[] = Data.artists[0].name
        .split('\n')
        .map((artist) => {
          return artist;
        });
      ...
      return (
        <Card
          key={'Music' + index}
          className="py-4 h-full w-full bg-cyan-950 max-w-[310px]"
        >
          <a
          ...
          >
            <CardHeader className="pb-0 pt-2 px-4 flex-col items-start">
              <p className="text-tiny text-slate-400 uppercase font-bold">
                {likesinger[0]}
              </p>
              <small className="text-default-500">No.{index + 1}</small>
              <h4 className="font-bold text-large text-slate-400">
                {songs[0]}
              </h4>
            </CardHeader>
